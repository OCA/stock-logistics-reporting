    ...
    def _create_materialized_view(self):
        ...
        self.env.cr.execute(
            ...
            CREATE MATERIALIZED VIEW %(table)s AS (
                ...
                -- Collect max and min date on all moves to avoid to collect all moves
                -- outside the observed scopes
                cfg_bounds AS (
                    SELECT
                ...
                -- Prefilter moves used to compute the consumption in order to avoid
                -- to process all moves of the system.
                -- Only moves with a date between the min and max bounds of the
                -- different configs are kept
                filtered_moves AS (
                    SELECT
                        sm.id,
                        sm.product_id,
                    ...
                    WHERE
                        ...
                        AND EXISTS (SELECT 1 FROM cfg_products cp
                            WHERE cp.product_id = sm.product_id)
                ),
                ...
                consumption AS (
                    ...
                    WHERE
                        ...
                        AND sl_dest.parent_path
                            NOT LIKE cfg.location_parent_path || '%%'
                ),
