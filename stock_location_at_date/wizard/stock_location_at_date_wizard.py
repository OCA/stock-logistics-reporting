...
class StockLocationAtDateWizard(models.TransientModel):
    ...
    def action_open_report(self):
        ...
        # Cost subquery for Odoo 17 ir_property model
        cost_subquery = """
            COALESCE(
                (
        ...
        query = f"""
            ...
            WHERE {where_clause}
            GROUP BY sml.company_id, loc.id, loc.complete_name, loc.usage, sml.product_id, pp.product_tmpl_id, pt.categ_id, pt.uom_id, sml.lot_id, sml.package_id, pp.id, pt.id
            HAVING SUM(
                CASE WHEN sml.location_dest_id = loc.id 
                     THEN sml.quantity_product_uom 
                     ELSE -sml.quantity_product_uom 
