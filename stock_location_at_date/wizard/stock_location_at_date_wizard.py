...
class StockLocationAtDateWizard(models.TransientModel):
    ...
    def action_open_report(self):
        ...
        # Build dynamic SQL WHERE filters
        where_conditions = [
            "sml.state = 'done'",
        ...
        query = f"""
            ...
            SELECT
                ...
                COALESCE((pp.standard_price->>sml.company_id::text)::numeric, 0.0) AS unit_cost,
                SUM(
                    CASE WHEN sml.location_dest_id = loc.id 
                         THEN sml.quantity_product_uom 
                         ELSE -sml.quantity_product_uom 
                    END
                ) * COALESCE((pp.standard_price->>sml.company_id::text)::numeric, 0.0) AS total_value
            FROM stock_move_line sml
            JOIN product_product pp ON pp.id = sml.product_id
            JOIN product_template pt ON pt.id = pp.product_tmpl_id
            JOIN stock_location loc ON loc.id IN (sml.location_id, sml.location_dest_id)
            WHERE {where_clause}
            GROUP BY sml.company_id, loc.id, loc.complete_name, loc.usage, sml.product_id, pp.product_tmpl_id, pt.categ_id, pt.uom_id, sml.lot_id, sml.package_id, pp.standard_price
            HAVING SUM(
                CASE WHEN sml.location_dest_id = loc.id 
                     THEN sml.quantity_product_uom 
                     ELSE -sml.quantity_product_uom 
