...
class ReportStockCardSummaryReportXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        for location in objects.location_ids:
            for ws_params in self._get_ws_params(workbook, data, location):
                ...
                generate_ws_method(workbook, ws, ws_params, data, objects, location)

    def _get_ws_params(self, wb, data, location):
        filter_template = {
            "1_date_from": {
        ...
        ws_params = {
            "ws_name": location.complete_name,
            "generate_ws_method": "_stock_card_report",
            "title": "Location - {}".format(location.complete_name),
    ...
    def _stock_card_report(self, wb, ws, ws_params, data, objects, location):
        ws.set_portrait()
        ws.fit_to_pages(1, 0)
