...
class TestStockPickingReportUndeliveredProduct(common.TransactionCase):
    ...
    def _transfer_picking_with_backorder(self, picking):
        action = picking.button_validate()
        backorder_wizard = self.BackOrderWiz.with_context(**action["context"]).create(
            {}
        )
    ...
    def test_displayed_customer(self):
        ...
        self.assertIn("undelivered_product_table", str(res[0]))

    def test_pending_backorder_moves(self):
        """Quantities postponed to a backorder are left to the standard block."""
        self.StockQuant.create(
            {
                "product_id": self.product_display.id,
                "location_id": self.warehouse.lot_stock_id.id,
                "quantity": 2000,
            }
        )
        ...
        ).move_line_ids.quantity = 20.00
        self._transfer_picking_with_backorder(picking)
        backorder = picking.backorder_ids
        self.assertTrue(backorder)
        # The backorder still has 30 units reserved but none of them delivered
        backorder_move = backorder.move_ids.filtered(
            lambda move: move.product_id == self.product_display
        )
        self.assertEqual(backorder_move.product_uom_qty, 30.00)
        self.assertEqual(backorder_move.quantity, 30.00)

        picking.company_id.undelivered_product_slip_report_method = "all"
        res = self.env["ir.actions.report"]._render_qweb_html(
            "stock.report_deliveryslip", picking.ids
        )
        self.assertNotIn("undelivered_product_table", str(res[0]))

    def test_pending_backorder_moves_partner_no_display(self):
        """Without the partner option the report is the standard one."""
        self.StockQuant.create(
            {
                "product_id": self.product_display.id,
                "location_id": self.warehouse.lot_stock_id.id,
    ...
    def test_picking_report_method(self):
        ...
        # Empty setting method field falls back to display every undelivered line
        picking.company_id.undelivered_product_slip_report_method = False
        res = self.env["ir.actions.report"]._render_qweb_html(
            "stock.report_deliveryslip", picking.ids
        )
        self.assertIn("test02", str(res[0]))
        self.assertIn("partially_undelivered_line", str(res[0]))
        self.assertIn("completely_undelivered_line", str(res[0]))
        ...
        self.assertIn("partially_undelivered_line", str(res[0]))
        self.assertNotIn("completely_undelivered_line", str(res[0]))
